  ...
  <div class="fixed bottom-2 left-1/2 -translate-x-1/2 z-50">
    <div
      ...
      <template v-for="(item, index) in items" :key="item.id">
        ...
        <button v-else @click="item.action" @mouseenter="hoveredIndex = index" @mouseleave="hoveredIndex = null"
          ...
          <transition name="fade">
            <div v-if="item.isOpen"
              class="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-1 h-1 bg-white rounded-full opacity-50"></div>
          </transition>
        </button>
      </template>
    </div>
